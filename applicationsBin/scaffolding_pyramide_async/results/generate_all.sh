...
generate_logs_if_required() {
    for i in `seq 2 9`; do
        if [[ ! -f simulation_async_"$i".log ]]; then
            ../meshAssembly -c ../b6/config_"$i"x"$i"_cf_b6.xml -t -R -l;
            mv simulation.log simulation_async_"$i".log
        fi
    done
}
...
generate_msg() {
    if [[ ! -f msg/msg_size.dat ]]; then
        ...
        for i in `seq 2 9`; do
            cat simulation_async_"$i".log | grep "nbMessages:" | \
    ...
    (cd msg \
         && gnuplot msg_async_all.plot \
         && read -p "add module count to msg_size and press a key to continue..." \
         && gnuplot msg_async_size.plot)
}

generate_mvmt() {
    for i in `seq 2 9`; do
        cat simulation_async_"$i".log | grep -e "mvmt: " > mvmt/mvmt.log;
        (cd mvmt \
...
generate_main() {
    echo -n "" > main.dat
    for i in `seq 2 9`; do
        cat simulation_async_"$i".log | grep -e "main: " | \
