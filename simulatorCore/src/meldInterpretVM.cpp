...
unsigned char * MeldInterpretVM::arguments = NULL;
	...
	void MeldInterpretVM::receive_tuple(int isNew, tuple_t tpl, byte face) {
            ...
            if(!TYPE_IS_LINEAR(type) && !TYPE_IS_ACTION(type)) {
                  ...
                  tuple_queue *queue = &(receivedTuples[face]);
                  if(isNew > 0) {
                        OUTPUT << __FILE__ << " Enqueuing new received tuple " << __LINE__ << endl;
                        tuple = malloc(tuple_size);
                        memcpy(tuple, rcvdTuple, tuple_size);
                        OUTPUT << " Enqueuing tuple of type " << (int)TUPLE_TYPE(tuple) << endl;
                        queue_enqueue(queue, tuple, (record_type)isNew);
                  } else {
                        OUTPUT << __FILE__ << " Enqueuing not new received tuple " << __LINE__ << endl;
...
/*#ifdef DEBUG_INSTRS
    ...
    void MeldInterpretVM::queue_push_tuple(tuple_queue *queue, tuple_entry *entry) {
          print_newTuples();
          OUTPUT << __FILE__ << " " << __LINE__ << " Queue_push_tuple() type is " << (int)TUPLE_TYPE(entry->tuple) << endl;
        if(queue->head == NULL){
            ...
            //OUTPUT << "head : " << (int)TUPLE_TYPE((_queue->head->tuple)) << endl;
            OUTPUT << __FILE__ << " " << __LINE__ << endl;
            OUTPUT << "entry tuple type " << (int)TUPLE_TYPE(queue->tail->tuple) << endl;
            //_queue->tail->next = (tuple_entry*)(entry->tuple);
            queue->tail->next = entry;
            OUTPUT << __FILE__ << " " << __LINE__ << endl;
            //_queue->tail = (tuple_entry*)(entry->tuple);
            queue->tail = entry;
            //OUTPUT << "entry tuple type " << (int)TUPLE_TYPE(_queue->tail->tuple) << endl;
            OUTPUT << "tail : " << queue->tail << " entry : " << entry << endl;
            OUTPUT << __FILE__ << " " << __LINE__ << endl;
        }
        print_newTuples();
    }
    ...
    tuple_t MeldInterpretVM::queue_dequeue_pos(tuple_queue *_queue, tuple_entry **pos) {
      ...
      tuple_entry* MeldInterpretVM::queue_enqueue(tuple_queue *_queue, tuple_t tuple, record_type isNew) {
            ...
            entry->records = isNew;
            entry->next = NULL;
            OUTPUT << __FILE__ << " " << __LINE__ << " queue_enqueue() type is " << (int)TUPLE_TYPE(entry->tuple) << endl;
