    ...
    void HexanodesWorld::createPopupMenu(int ix, int iy) {
        ...
        if (nbreMenus == 0) {
            ...
            for (auto &motion: tab) {
                ...
                lattice->getCellInDirection(bb->position,
                                            (motion->direction == motionDirection::CW ? (motion->fromConId + 1) % 6 :
                                             (motion->fromConId + 5) % 6));
                ...
                if (lattice->isInGrid(finalPos) && lattice->isFree(finalPos)) {
                    rotateBlockSubMenu->addButton(
                            new GlutRotation2DButton(nullptr, i++, 0, 0, 0, 0,
                                                     menuTextureDirectory + "menu_link_hexanodes.tga",
                                                     motion->direction == CCW, motion->direction == CCW, 2, finalPos,
                                                     orient, 0.06)
                    );
                }
            }
 ...
 */
    ...
    void HexanodesWorld::linkBlock(const Cell3DPosition &pos) {
        ...
        for (int i = 0; i < HHLattice::MAX_NB_NEIGHBORS; i++) {
            ...
            if (neighborBlock) {
                module->getInterface(HHLattice::Direction(i))->connect(
                        neighborBlock->getInterface(HHLattice::Direction(lattice->getOppositeDirection(i))));
...
/*    BuildingBlock *bb = getSelectedBuildingBlock() ?: getMap().begin()->second;
    ...
    void HexanodesWorld::glDrawBackground() {
        ...
        glVertex3f((lattice->gridSize[0] + 0.5 * lattice->gridSize[1]) * lattice->gridScale[0],
                   lattice->gridSize[1] * lattice->gridScale[1] * M_SQRT3_2, 0.0f);
        glTexCoord2f(0.25 * lattice->gridSize[1], 0.5 * lattice->gridSize[1]);
        glVertex3f(0.5 * lattice->gridSize[1] * lattice->gridScale[0],
                   lattice->gridSize[1] * lattice->gridScale[1] * M_SQRT3_2, 0.0f);
        glEnd();
        glPopMatrix();
    }
