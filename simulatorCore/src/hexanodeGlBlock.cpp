...
#include "hexanodeGlBlock.h"
#include "hexanodeWorld.h"
#include "lattice.h"

namespace Hexanode {

HexanodeGlBlock::HexanodeGlBlock(bID id):GlBlock(id), displayedValue(id) {
}

void HexanodeGlBlock::glDraw(ObjLoader::ObjLoader *ptrObj) {
	glPushMatrix();
...
// glMaterialfv(GL_FRONT,GL_AMBIENT_AND_DIFFUSE,color);
	...
	if (displayedValue>=0) {
		...
		HexanodeWorld *wrld = (HexanodeWorld*)getWorld();
		enableTexture(true);
...
void HexanodeGlBlock::glDrawShadows(ObjLoader::ObjLoader *ptrObj) {
	glPushMatrix();
	mat.glMultMatrix();
	ptrObj->glDraw();
	glPopMatrix();
...
void HexanodeGlBlock::glDrawId(ObjLoader::ObjLoader *ptrObj,int n) {
    glPushMatrix();
		mat.glMultMatrix();
		ptrObj->glDrawId(n);
    glPopMatrix();
...
void HexanodeGlBlock::glDrawIdByMaterial(ObjLoader::ObjLoader *ptrObj,int &n) {
    glPushMatrix();
		mat.glMultMatrix();
