...
Clock::Clock(ClockType clockType, BuildingBlock *h) {
    ...
    noiseGenerator =  std::ranlux48(hostBlock->blockId);
...
void Clock::setClockProperties(ClockType clockType) {
    ...
    case XMEGA_RTC_OSC1K_CRC:
    {
        ...
        for (int i = 0; i < 3; i++) {
            std::mt19937 uGenerator(hostBlock->blockId);
            std::normal_distribution<double> normalDist(mean[i],sd[i]);
            //<! Equivalent to boost::variate_generator according to:
            //  http://stackoverflow.com/questions/13691813/stdvariate-generator-in-vs2012
...
uint64_t Clock::getTimeUS(uint64_t simTime) {
    ...
    std::mt19937 uGenerator(simTime*hostBlock->blockId);
...
uint64_t Clock::getSchedulerTimeForLocalTime(uint64_t localTime) {
    ...
    std::mt19937 uGenerator(localTime*hostBlock->blockId);
    std::normal_distribution<double> normalDist(0,sigma);
