...
C2DMeldBlockCode::C2DMeldBlockCode(Catoms2DBlock *host): Catoms2DBlockCode(host) {
	OUTPUT << "C2DMeldBlockCode constructor" << endl;
	vm = new MeldInterpretVM((Catoms2DBlock*)hostBlock);
	hasWork = true; // mode fastest
	polling = false; // mode fastest
	currentLocalDate = 0; // mode fastest
	bb = (Catoms2DBlock*) hostBlock;

	// if((vm != NULL)) {
	// 	for (int i = 0; i < host->getNbInterfaces(); i++) {
	// 		vm->neighbors[i] = vm->get_neighbor_ID(i);
	// 		cout << "Adding neighbor " << vm->neighbors[i] << " on face " << i << endl;
...
C2DMeldBlockCode::~C2DMeldBlockCode() {
	delete vm;
	OUTPUT << "C2DMeldBlockCode destructor" << endl;
}

void C2DMeldBlockCode::init() {
	stringstream info;
...
void C2DMeldBlockCode::startup() {
	stringstream info;

	currentLocalDate = BaseSimulator::getScheduler()->now();
	info << "  Starting C2DMeldBlockCode in block " << hostBlock->blockId;
	getScheduler()->trace(info.str(),hostBlock->blockId);
	init();
}

void C2DMeldBlockCode::handleDeterministicMode(/*VMCommand &command*/){
...
void C2DMeldBlockCode::processLocalEvent(EventPtr pev) {
	stringstream info;
	assert(vm != NULL);
	info.str("");
	...
	case EVENT_ADD_NEIGHBOR:
	{
		...
		info << "Add neighbor "<< (std::static_pointer_cast<AddNeighborEvent>(pev))->target
			 << " at face "
			 << HLattice::getString(HLattice::getOpposite(
	...
	case EVENT_REMOVE_NEIGHBOR:
	{
		...
		BaseSimulator::getScheduler()->schedule(
			new ComputePredicateEvent(BaseSimulator::getScheduler()->now(), bb));
		info << "Remove neighbor at face " << HLattice::getString(HLattice::getOpposite(face));
	}
	break;
	case EVENT_TAP:
	{
