...
void TetrisCode::sendTmn7(bool reinit, bool blocked)
{
...
void TetrisCode::myTmn7Func(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender)
{
    ...
    if (msgData.stage >= stage && update < msgData.nbupdate && (tmn != 7 || rotation != msgData.rotation || position != msgData.position || color != msgData.color))
    {
        stage = msgData.stage;
        tmn = 7;
...
void TetrisCode::myRestartTmn7Func(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender)
{
    ...
    else if (msgData.stage >= stage)
    {
        ...
        {
            sendMessage("Restart Tmn 7 Message", new MessageOf<TmnData>(START_TMN7_MSG_ID, msgData), i, 0, 0);
        }
    }
}

void TetrisCode::verifTmn7()
{
...
void TetrisCode::sendVerifTmn7(bool answer, isFreeData data)
{
    P2PNetworkInterface *i = itf[westId];
    if ((position == 1 || !westBool) && i != nullptr && i->isConnected())
    {
        if (answer)
        {
