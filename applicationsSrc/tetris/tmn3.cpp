...
void TetrisCode::sendTmn3(bool reinit, bool blocked)
{
...
void TetrisCode::myTmn3Func(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender)
{
    ...
    if (msgData.stage >= stage && update < msgData.nbupdate && (tmn != 3 || rotation != msgData.rotation || position != msgData.position || color != msgData.color))
    {
        stage = msgData.stage;
        tmn = 3;
...
void TetrisCode::myRestartTmn3Func(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender)
{
    ...
    else if (msgData.stage >= stage)
    {
        ...
        {
            sendMessage("Restart Tmn 3 Message", new MessageOf<TmnData>(START_TMN3_MSG_ID, msgData), i, 0, 0);
        }
    }
}

void TetrisCode::verifTmn3()
{
