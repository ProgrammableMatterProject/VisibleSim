...
void RoutableScaffoldMessage::route(BaseSimulator::BlockCode *bc) {
    ...
    if (not mabc.ruleMatcher->isStructuralPosition(mabc.norm(mabc.catom->position)));
    ...
    bool dstInMesh = mabc.ruleMatcher->isStructuralPosition(mabc.norm(dstPos));
    ...
    if (not dstInMesh) {
        for (const Cell3DPosition& nPos : mabc.lattice->getActiveNeighborCells(dstPos)) {
            if (mabc.ruleMatcher->isStructuralPosition(mabc.norm(nPos))
        ...
        VS_ASSERT(dstSFPos != Cell3DPosition(numeric_limits<short>::max(),
                                             numeric_limits<short>::max(),
                                             numeric_limits<short>::max())
                  and mabc.ruleMatcher->isStructuralPosition(mabc.norm(dstSFPos)));
    }
    ...
    if (mabc.role != FreeAgent) { // module is pivot
        ...
        if (targetLightNeighbor
            ...
            if (mabc.greenLightIsOn
                ...
                if (mabc.catom->position[2] >= mabc.scaffoldSeedPos[2])
