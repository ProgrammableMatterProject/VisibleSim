...
SBMeldBlockCode::SBMeldBlockCode(SmartBlocksBlock *host): SmartBlocksBlockCode(host) {
	OUTPUT << "SBMeldBlockCode constructor" << endl;
	vm = new MeldInterpretVM(hostBlock);
	hasWork = true; // mode fastest
	polling = false; // mode fastest
	currentLocalDate = 0; // mode fastest
	bb = (SmartBlocksBlock*) hostBlock;
}

SBMeldBlockCode::~SBMeldBlockCode() {
	delete vm;
	OUTPUT << "SBMeldBlockCode destructor" << endl;
}

void SBMeldBlockCode::init() {
	stringstream info;
...
void SBMeldBlockCode::startup() {
	stringstream info;

	currentLocalDate = BaseSimulator::getScheduler()->now();
	info << "  Starting SBMeldBlockCode in block " << hostBlock->blockId;
	getScheduler()->trace(info.str(),hostBlock->blockId);
	init();
}

void SBMeldBlockCode::handleDeterministicMode(/*VMCommand &command*/){
...
void SBMeldBlockCode::processLocalEvent(EventPtr pev) {
	stringstream info;
	assert(vm != NULL);
	info.str("");
