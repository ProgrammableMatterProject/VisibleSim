...
/**
   ...
   @remarks It's rather doubtful whether this function might be of any use, as @ref{getAllLinks} might need to be used by default */
set<Catoms3DMotionRulesLink> getAllLinksToConnector(const Catoms3DBlock *pivot,
                                                    short conId);
...
set<Catoms3DMotionRulesLink> getAllLinks(const Catoms3DBlock *pivot);
...
list<Catoms3DMotionRulesLink> findConnectorsPath(const set<Catoms3DMotionRulesLink>& motionRulesLinks,
                                                 short conFrom,
                                                 short conTo);

/**
   @brief Given a set of motion rules link passed as argument, deduce a set of all connectors for which a path exists between it and connector conTo
   @param motionRulesLinks a set of surface links between connectors of a pivot module that another module can follow to rotate
...
set<short> findPathConnectors(const set<Catoms3DMotionRulesLink>& motionRulesLinks,
                              short conTo);

/**
   @brief Given a set of connector IDs and the orientation of the module to which they belong, determine which connectors of the current module are adjacent to those contained in the input set. Two connectors of opposing modules are adjacent if a third module could fill a position where both connectors are connected to that module at the same time
...
set<short> findAdjacentConnectors(const set<short>& pathConnectors,
...
/**
   ...
   @param position a cell position that belongs to the simulated lattice
...
short getConnectorAdjacentToPosition(Catoms3DBlock *catom, const Cell3DPosition position);
