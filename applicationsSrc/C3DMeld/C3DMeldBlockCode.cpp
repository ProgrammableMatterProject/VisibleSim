...
C3DMeldBlockCode::C3DMeldBlockCode(Catoms3DBlock *host): Catoms3DBlockCode(host) {
	OUTPUT << "C3DMeldBlockCode constructor" << endl;
	vm = new MeldInterpretVM((Catoms3DBlock*)hostBlock);
	hasWork = true; // mode fastest
	polling = false; // mode fastest
	currentLocalDate = 0; // mode fastest
	bb = (Catoms3DBlock*) hostBlock;
}

C3DMeldBlockCode::~C3DMeldBlockCode() {
	delete vm;
	OUTPUT << "C3DMeldBlockCode destructor" << endl;
}

void C3DMeldBlockCode::init() {
	stringstream info;
...
void C3DMeldBlockCode::startup() {
	stringstream info;

	currentLocalDate = BaseSimulator::getScheduler()->now();
	info << "  Starting C3DMeldBlockCode in block " << hostBlock->blockId;
	getScheduler()->trace(info.str(),hostBlock->blockId);
	init();
}

void C3DMeldBlockCode::handleDeterministicMode(/*VMCommand &command*/){
...
void C3DMeldBlockCode::processLocalEvent(EventPtr pev) {
	stringstream info;
	assert(vm != NULL);
	info.str("");
