...
void Blinky01BlockCode::processLocalEvent(EventPtr pev) {
    ...
    case EVENT_REMOVE_NEIGHBOR:
    {
        ...
        sendCommand(command);
        info << "Remove neighbor at face " << lattice->getDirectionString(lattice->getOppositeDirection((std::static_pointer_cast<AddNeighborEvent>(pev))->face));
    }
    break;
    case EVENT_TAP:
    {
    ...
    case EVENT_SEND_MESSAGE:
    {
        ...
        MeldProcess::getScheduler()->schedule(new NetworkInterfaceEnqueueOutgoingEvent(BaseSimulator::getScheduler()->now(),
                                                                                       message, interface));
        info << "sends a message at face " << lattice->getDirectionString(bb->getDirection(interface))  << " to " << interface->connectedInterface->hostBlock->blockId;
    }
    break;
    case EVENT_RECEIVE_MESSAGE: /*EVENT_NI_RECEIVE: */
    {
        ...
        sendCommand(*command);
        info << "message received at face " << lattice->getDirectionString(bb->getDirection(command->sourceInterface->connectedInterface)) << " from " << command->sourceInterface->hostBlock->blockId;
    }
    break;
    case EVENT_ACCEL:
    {
...
int Blinky01BlockCode::sendCommand(VMCommand &c) {
    ...
    lockVM();
    if(vm != NULL) {
        if ((hostBlock->getState() == BuildingBlock::ALIVE) || (c.getType() == VM_COMMAND_STOP)) {
