...
void MeshAssemblyBlockCode::onBlockSelected() {
    ...
    cout << "norm(pos): " << norm(catom->position) << endl;
...
void MeshAssemblyBlockCode::startup() {
    ...
    if (catom->position == (getEntryPointForMeshComponent(MeshComponent::R)
        ...
        short bi = ruleMatcher->determineBranchForPosition(sbnorm(catom->position));
...
const Cell3DPosition MeshAssemblyBlockCode::getEntryPointForMeshComponent(MeshComponent mc) {
    switch(mc) {
        ...
        case X_2: case X_3: case X_4: case X_5: case X_6: case X_7: return getEntryPointPosition(RZ_EPL);

        case Y_1: return getEntryPointPosition(Z_L_EPL);
        case Y_2: case Y_3: case Y_4: case Y_5: case Y_6: case Y_7: return getEntryPointPosition(LZ_EPL);
...
void MeshAssemblyBlockCode::buildConstructionQueue() {
    ...
    if (catomsReqByBranch[YBranch] != -1) constructionQueue.push_back({ Y_6, LZ_EPL }); // 11
    if (catomsReqByBranch[XBranch] != -1) constructionQueue.push_back({ X_6, RZ_EPL }); // 11
    if (catomsReqByBranch[XBranch] != -1) constructionQueue.push_back({ X_7, RZ_EPL }); // 11
    if (catomsReqByBranch[YBranch] != -1) constructionQueue.push_back({ Y_7, LZ_EPL }); // 11
    ...
    if (catomsReqByBranch[ZBranch] != -1) constructionQueue.push_back({ Z_6, Z_EPL }); // 16
    if (catomsReqByBranch[ZBranch] != -1) constructionQueue.push_back({ Z_7, Z_EPL }); // 17
    if (catomsReqByBranch[RevZBranch] != -1) constructionQueue.push_back({ RevZ_5, RevZ_EPL }); // 16
    if (catomsReqByBranch[RevZBranch] != -1) constructionQueue.push_back({ RevZ_6, RevZ_EPL }); // 16
        if (catomsReqByBranch[RevZBranch] != -1) constructionQueue.push_back({ RevZ_7, RevZ_EPL }); // 17
    ...
    if (catomsReqByBranch[LZBranch] != -1) constructionQueue.push_back({ LZ_6, LZ_EPL }); // 22
    if (catomsReqByBranch[LZBranch] != -1) constructionQueue.push_back({ LZ_7, LZ_EPL }); // 22
    if (catomsReqByBranch[RZBranch] != -1) constructionQueue.push_back({ RZ_5, RZ_EPL }); // 22
    if (catomsReqByBranch[RZBranch] != -1) constructionQueue.push_back({ RZ_6, RZ_EPL }); // 22
    if (catomsReqByBranch[RZBranch] != -1) constructionQueue.push_back({ RZ_7, RZ_EPL }); // 22
