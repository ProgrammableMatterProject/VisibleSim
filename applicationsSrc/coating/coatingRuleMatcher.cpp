/**
 * @file   coatingRuleMatcher.cpp
 * @author pthalamy <pthalamy@p3520-pthalamy-linux>
 * @date   Thu Oct 10 10:07:22 2019
 *
 * @brief
 *
 *
...
std::array<Cell3DPosition, N_COMPONENTS> CoatingRuleMatcher::componentPosition = {
...
string CoatingRuleMatcher::component_to_string(ScafComponent comp) {
    switch(comp) {
        case R: return "R";
        case S_Z: return "S_Z";
...
Cell3DPosition CoatingRuleMatcher::getPositionForComponent(ScafComponent comp) {
    return comp < N_COMPONENTS ? componentPosition[comp] : Cell3DPosition();
}

int CoatingRuleMatcher::getComponentForPosition(const Cell3DPosition& pos) {
...
ScafComponent CoatingRuleMatcher::getDefaultEPLComponentForBranch(BranchIndex bi) {
    switch(bi) {
...
ScafComponent CoatingRuleMatcher::getTargetEPLComponentForBranch(BranchIndex bi) {
    switch(bi) {
...
void CoatingRuleMatcher::printDebugInfo(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isInGrid(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isInSandbox(const Cell3DPosition& pos) const {
    return isInMesh(pos) and pos[2] < 0;
}

bool CoatingRuleMatcher::isInMesh(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isOnXBranch(const Cell3DPosition& pos) const {
    return m_mod(pos[1], B) == 0 and m_mod(pos[2], B) == 0
        and not isOnOppXBranch(pos);
}

bool CoatingRuleMatcher::isOnOppXBranch(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isOnYBranch(const Cell3DPosition& pos) const {
    return m_mod(pos[0], B) == 0 and m_mod(pos[2], B) == 0
        and not isOnOppYBranch(pos);
}

bool CoatingRuleMatcher::isOnOppYBranch(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isOnZBranch(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isOnRevZBranch(const Cell3DPosition& pos) const {
    const int x = (pos[0] < 0 ? B + pos[0] : pos[0]);
    const int y = (pos[1] < 0 ? B + pos[1] : pos[1]);
    const int z = pos[2];
...
bool CoatingRuleMatcher::isOnLZBranch(const Cell3DPosition& pos) const {
    const int x = (pos[0] < 0 ? B + pos[0] : pos[0]);
    const int y = (pos[1] < 0 ? B + pos[1] : pos[1]);
    const int z = pos[2];
...
bool CoatingRuleMatcher::isOnRZBranch(const Cell3DPosition& pos) const {
    const int x = (pos[0] < 0 ? B + pos[0] : pos[0]);
    const int y = (pos[1] < 0 ? B + pos[1] : pos[1]);
    const int z = pos[2];
...
bool CoatingRuleMatcher::isVerticalBranchTip(const Cell3DPosition& pos) const {
    return isNFromVerticalBranchTip(pos, 0);
}

bool CoatingRuleMatcher::isNFromVerticalBranchTip(const Cell3DPosition& pos, int N) const {
    return (isOnZBranch(pos) or isOnRevZBranch(pos)
            or isOnLZBranch(pos) or isOnRZBranch(pos)) and m_mod(pos[2], B) == (B - N - 1);
}

bool CoatingRuleMatcher::isEPLPivotModule(const Cell3DPosition& pos) const {
    return isNFromVerticalBranchTip(pos, 1);
}

bool CoatingRuleMatcher::isBranchModule(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isZBranchModule(const Cell3DPosition& pos) const {
    return (not isTileRoot(pos)) and (isOnZBranch(pos) or isOnRevZBranch(pos)
            or isOnLZBranch(pos) or isOnRZBranch(pos));
}

bool CoatingRuleMatcher::isSupportModule(const Cell3DPosition& pos) const {
...
BranchIndex
CoatingRuleMatcher::getBranchIndexForNonRootPosition(const Cell3DPosition& pos) const{
...
bool CoatingRuleMatcher::shouldGrowBranch(const Cell3DPosition& pos,
                                              BranchIndex bi) const {
    return resourcesForBranch(pos, bi) > 0;
}

short CoatingRuleMatcher::resourcesForBranch(const Cell3DPosition& pos,
                                                 BranchIndex bi) const {
...
Cell3DPosition CoatingRuleMatcher::getBranchUnitOffset(int bi) const {
    switch(bi) {
...
Cell3DPosition CoatingRuleMatcher::getBranchUnitOffset(const Cell3DPosition& pos) const {
    return getBranchUnitOffset(getBranchIndexForNonRootPosition(pos));
}

bool CoatingRuleMatcher::isTileRoot(const Cell3DPosition& pos) const {
...
CoatingRuleMatcher::getNearestTileRootPosition(const Cell3DPosition& pos) const {
    int trX;
...
const Cell3DPosition
CoatingRuleMatcher::getTileRootPositionForMeshPosition(const Cell3DPosition& pos) const {
    // cout << "pos: " << pos << endl;
    if (isSupportModule(pos)) {
        if (isTileRoot(pos - getPositionForComponent(S_RevZ)))
...
bool CoatingRuleMatcher::isInTileWithRootAt(const Cell3DPosition& root,
                                                const Cell3DPosition& pos) const {
    return root == getTileRootPositionForMeshPosition(pos);
}

const Cell3DPosition
CoatingRuleMatcher::getSupportPositionForPosition(const Cell3DPosition& pos) const {
...
AgentRole CoatingRuleMatcher::getRoleForPosition(const Cell3DPosition& pos) const {
...
const Cell3DPosition CoatingRuleMatcher::getPositionForScafComponent(ScafComponent mc) {
    switch(mc) {
...
int CoatingRuleMatcher::getBranchIndexForScafComponent(ScafComponent mc) {
    switch(mc) {
        case X_1: case X_2: case X_3: case X_4: case X_5:
            return XBranch;
...
const Cell3DPosition
CoatingRuleMatcher::getPositionForChildTileScafComponent(ScafComponent mc) const {
    switch(mc) {
        // case EPLs
        // case RevZ_EPL: return Cell3DPosition(-1,-1,-1);
        // case RevZ_R_EPL: return Cell3DPosition(0,-1,-1);
...
const Color& CoatingRuleMatcher::getColorForPosition(const Cell3DPosition& pos) const {
...
Cell3DPosition CoatingRuleMatcher::getPositionOfBranchTipUnder(BranchIndex bi) {
    switch(bi) {
...
BranchIndex CoatingRuleMatcher::getAlternateBranchIndex(BranchIndex bi) const {
    switch(bi) {
...
string CoatingRuleMatcher::roleToString(AgentRole ar) {
    switch(ar) {
...
string CoatingRuleMatcher::branch_to_string(BranchIndex bi) {
    switch(bi) {
...
BranchIndex CoatingRuleMatcher::getBranchForEPL(ScafComponent epl) {
    switch(epl) {
...
const Cell3DPosition CoatingRuleMatcher::getTargetEPLPositionForBranch(BranchIndex bi) {
    switch (bi) {
...
const Cell3DPosition
CoatingRuleMatcher::getTileRootAtEndOfBranch(const Cell3DPosition& trRef,
                                          BranchIndex bi,
                                          bool upward) const {
...
const Cell3DPosition
CoatingRuleMatcher::getEntryPointPosition(const Cell3DPosition& cPos,
                                              ScafComponent epl) const {
    return getEntryPointRelativePos(epl) + cPos;
}


bool CoatingRuleMatcher::isOnHorizontalEPL(const Cell3DPosition& pos) const {
...
bool CoatingRuleMatcher::isOnEntryPoint(const Cell3DPosition& pos) const {
    const Cell3DPosition& nearestTR = getNearestTileRootPosition(pos);
...
short CoatingRuleMatcher::getEntryPointLocationForCell(const Cell3DPosition& pos) {
    for (int i = 0; i < 12; i++)
...
const Cell3DPosition
CoatingRuleMatcher::getEntryPointForScafComponent(const Cell3DPosition& cPos,
                                                      ScafComponent mc) {
    switch(mc) {
...
const Cell3DPosition
CoatingRuleMatcher::getEntryPointForModuleOnIncidentBranch(const Cell3DPosition& cPos,
                                                               BranchIndex bid) {
    ScafComponent epl;
    switch(bid) {
...
bool CoatingRuleMatcher::areOnTheSameBranch(const Cell3DPosition& pos1,
...
bool CoatingRuleMatcher::CSGTRAtBranchTipShouldGrowBranch(const Cell3DPosition& pos,
                                                              BranchIndex tipB,
...
Cell3DPosition CoatingRuleMatcher::getSeedForCSGLayer(int z) const {
...
BranchIndex CoatingRuleMatcher::
