...
bool Border::isPlaneSeed(const Cell3DPosition& pos) const {
...
bool Border::couldBeSeed(const Cell3DPosition& pos) const {
    const Cell3DPosition& zPos = pos + Cell3DPosition(0,0,1);
    if (isInG(pos) and isInG(zPos))
        if (isOnBorder(pos) or isOnBorder(zPos))
            return true;
    return false;
}

bool Border::isSeedBorderOnCurrentPlane(const Cell3DPosition& pos) const {
    return isInG(pos.addZ(1)) and isOnBorder(pos) and isLowestOfBorderOnPlane(pos);
}

bool Border::isSeedBorderOnNextPlane(const Cell3DPosition& pos) const {
    return isInG(pos) and isOnBorder(pos) and isLowestOfBorderOnPlane(pos, true);
}

bool Border::isLowestOfBorderOnPlane(const Cell3DPosition& pos, bool next) const {
    int nTurns = 0;
    int idx = getIndexForBorder(pos);
    Cell3DPosition currentPos = pos;
