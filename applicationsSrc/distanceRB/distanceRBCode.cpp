...
void DistanceRBCode::startup() {
	...
	if (module->blockId==1) { // master id is 1
		distance=0;
		setColor(RED);
		sendMessageToAllNeighbors("Broadcast",new MessageOf<uint16_t>(BROADCAST_MSG,0),200,50,0);
	} else {
		distance=UINT16_MAX;
		setColor(LIGHTGREY);
	}
...
void DistanceRBCode::myBROADCAST_MSGFunc(const MessageOf<uint16_t>*msg, P2PNetworkInterface*sender) {
	...
	if (distance>d) {
		...
		sendMessageToAllNeighbors("Broadcast",new MessageOf<uint16_t>(BROADCAST_MSG,distance),200,50,1,sender);
	}
};
