...
void Sync::handleResponseLeft(shared_ptr<Sync_response_message> msg)
{
    ...
    if (reconf->isSeedNext() && reconf->getNumberSeedsLeft() && !route->leftSeedVisited && route->direction != TO_LEFT) {
        route->leftSeedVisited = true;
        syncLeft->syncSeed(msg->syncModel, TO_PREVIOUS);
    }
    else if (reconf->isLineParent() && route->leftSeedVisited && route->direction != TO_PREVIOUS) {
