...
Sync::Sync(Catoms3D::Catoms3DBlock *c, Reconf *r) : catom(c), reconf(r)
{
    ...
    syncRight = new SyncRight(catom, reconf, syncResponseModel, syncResponse);
}
...
void Sync::handleResponse(MessagePtr message) {
    ...
    else if (recv_message->toLeft)
        handleResponseLeft(recv_message);
    else
        handleResponseRight(recv_message);
}
...
void Sync::handleResponseLeft(shared_ptr<Sync_response_message> msg)
{
    ...
    else if (reconf->isLineParent() && route->leftSeedVisited) {
...
void Sync::handleLookupNeighborRightMessage(MessagePtr message)
{
    shared_ptr<Lookup_forward_right_sync_message> recv_message = static_pointer_cast<Lookup_forward_right_sync_message>(message);
    ...
    syncRight->syncNeighbor(recv_message->syncInfo, recv_message->side_direction, recv_message->line_direction);
}
...
void Sync::handleLookupLineRightMessage(MessagePtr message)
{
    shared_ptr<Lookup_line_right_sync_message> recv_message = static_pointer_cast<Lookup_line_right_sync_message>(message);
    ...
    syncRight->syncSeed(recv_message->syncInfo, recv_message->lineDirection);
}
